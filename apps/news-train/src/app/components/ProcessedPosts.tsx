...
const ProcessedPosts: FunctionComponent<Props> = ({children}: Props) => {
  ...
  const { data: stacksFilenames } = useSWR('stacksFilenames', fetchStacksFilenames, {suspense: true})


  const fetcher = () => {
    console.log('fetcher')
    if( !userSession.isUserSignedIn() ) {
      localforage.getItem(processedFilenameForFeed)
      .then((value) => {
        if (!value) {
    ...
    [stacksFilenames].flat().filter((stacksFilename) => {
      ...
      console.log('getFile')
      storage.getFile(`${stacksFilename}`, {
        decrypt: true
      })
      .then((content) => {
