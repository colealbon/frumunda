...
export function useSettings () {
  ...
  const persistSettings = useCallback((newSettings: unknown) => {
    setInFlight(true)
    const newSettingsClone = structuredClone(newSettings as object)
    
    const updateFn = (newSettings: object) => {
      return new Promise((resolve) => {
        localforage.setItem('settings', newSettingsClone)
        .then(() => {
          setInFlight(false)
    ...
    const options = { optimisticData: defaultSettings, rollbackOnError: false }
