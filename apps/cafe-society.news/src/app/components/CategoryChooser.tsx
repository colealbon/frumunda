      ...
      <ListItem key={'category_chooser_allcategories'} disablePadding>
        <ListItemButton
          ...
          onClick={() => {
            ...
            mutate('selectedPage', persistLocal('selectedPage', 'posts'), {
              optimisticData: 'posts',
            });
          }}
        >
          <ListItemText
            key={'CategoryChooserAllCategories'}
            primary={'posts'}
          />
        </ListItemButton>
      </ListItem>
      ...
      <Collapse in={true} timeout="auto" unmountOnExit>
        <List component="div" disablePadding>
          {Object.entries({ ...(categories as object) })
            .filter(
              (categoryItem) =>
                Object.assign(categoryItem[1] as object).checked === true
            )
            .map((categoryItem) => {
              ...
              return (
                <ListItem
                  key={`category_chooser_${categoryItem[0]}`}
                  disablePadding
                >
                  <ListItemButton
                    sx={{ pl: 4 }}
